...
public class StarcraftAssistantGUI extends javax.swing.JFrame {
    ...
    public StarcraftAssistantGUI() {// Creates new form GUI
        initComponents();
    } 

    // this updates the image of the map
    public void update() {
//        OntologyQueryInterface jenaOntologyModel = BWAPIParser.initiateParser();
//        updateEnemyAbilities(jenaOntologyModel);
//        updateRegionStates(jenaOntologyModel); 
//        updateProgressBar(jenaOntologyModel);
    }
    ...
    public void updateEnemyAbilities() {
        ...
        ResultSet results = OntologyInfo.getInstance().queryEnemyAbilities();
        ...
        while (results.hasNext()) {
            ...
            if (soln.getResource("p").toString().contains(matchTrain.subSequence(0, matchTrain.length()))) {
                ...
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("marine")) {
                    marine.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("firebat")) {
                    firebat.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("medic")) {
                    medic.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("ghost")) {
                    ghost.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("vulture")) {
                    vulture.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("tank")) {
                    tank.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("goliath")) {
                    goliath.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("wraith")) {
                    wraith.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("dropship")) {
                    dropship.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("scienceVessel")) {
                    scienceVessel.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("battleCruiser")) {
                    battleCruiser.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("valkyrie")) {
                    valkyrie.setForeground(c);
                }
            }
            if (soln.getResource("p").toString().contains(matchBuild.subSequence(0, matchBuild.length()))) {
                Color c = new Color(0, 0, 0);
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("factory")) {
                    factor.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("engineeringBay")) {
                    engbay.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("supplyDepot")) {
                    supplyDepot.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("commandCenter")) {
                    commandCenter.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("bunker")) {
                    bunker.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("armory")) {
                    armory.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("academy")) {
                    academy.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("refinery")) {
                    refinery.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("scienceFacility")) {
                    science.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("starport")) {
                    starport.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("missleTurret")) {
                    turret.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("comsatStation")) {
                    comsat.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("machineShop")) {
                    machineshop.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("controlTower")) {
                    controltower.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("physicsLab")) {
                    physicslab.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("nuclearSilo")) {
                    nuclearsilo.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("covertOps")) {
                    covertops.setForeground(c);
                }
                if (soln.getResource("o").toString().replace(OntologyInfo.NS, "").replace("Available", "").equals("barracks")) {
    ...
    public void updateRegionStates() {
        ResultSet resultsC = OntologyInfo.getInstance().queryContestedRegions();
        ...
        ResultSet resultsB = OntologyInfo.getInstance().queryBattleRegions();
        ...
        try {
            ...
            mapImageOriginal = ImageIO.read(new File(Useful.getPropValue("GUI_SC_MAP_FILE")));
        } catch (IOException ex) {
            System.out.append("Could not read original map image from file");
            Logger.getLogger(StarcraftAssistantGUI.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        int mapHeight = 96 * 32;
        int mapWidth = 128 * 32;
        ResultSet myUnitsResults = OntologyInfo.getInstance().queryMyUnits();
        ...
        ResultSet enemyUnitsResults = OntologyInfo.getInstance().queryEnemyUnits();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(StarcraftAssistantGUI.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new StarcraftAssistantGUI().setVisible(true);
            }
        });
    }
    ...
    private void updateProgressBar() {
        ResultSet results = OntologyInfo.getInstance().queryArmyHealth();
        int runningCurrHP = 0;
        int runningMaxHP = 0;
        while (results.hasNext()) {
            QuerySolution soln = results.nextSolution();
