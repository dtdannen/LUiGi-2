        ...
        switch (this.expectationName) {
            case HAVE_UNITS:
                ...
                for (Pair<String, Integer> unitTypeCount : unitTypeCounts) {
                    ...
                    int realCount = OntologyInfo.getInstance().countUnitsOfTypeOfPlayer(unitTypeCount.getFirst(), ProjectSettings.myPlayerID);
            ...
            case HAVE_UNITS_WITH_IDS:
                ...
                break;
            case CONTROL_REGION:
                expectationMet = OntologyInfo.getInstance().doesPlayerControlRegionSPARQL(ProjectSettings.myPlayerID, regionID);
                //expectationMet = OntologyInfo.getInstance().doesPlayerControlRegionPELLET(ProjectSettings.myPlayerID, regionID);

                if (expectationMet == false) {
                    // check to see if we have any units in the region
                    if (OntologyInfo.getInstance().isRegionContested(regionID)) {
                        ...
                        this.explanation.setUnitIDs(OntologyInfo.getInstance().getFightingUnitIDsInRegion(GlobalInfo.getEnemyPlayerID(), regionID));
                        if (this.explanation.getUnitIDs().size() > 0) {
                            //JOptionPane.showMessageDialog(null, "There is more than 1 unit!!");
                        }
                    } else if (OntologyInfo.getInstance().getFightingUnitIDsInRegion(ProjectSettings.myPlayerID,regionID).isEmpty())  {
            ...
            case UNITS_IN_REGION:
                ...
                ArrayList<Integer> unitIdsInRegion = OntologyInfo.getInstance().getUnitIDsInRegion(regionID);
                int count = 0;
