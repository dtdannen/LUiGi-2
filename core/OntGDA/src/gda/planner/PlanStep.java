...
public class PlanStep {
    ...
    // maintains whether or not this plan step has been sent to the bot already (so that we don't send twice)
    private boolean sentToBot = false;
    ...
    public String toString() {
        String result = this.plan.getID() + " " + this.planStepType.name() + " ";
        ...
        switch (this.planStepType) {
            ...
            case ATTACK_MOVE_UNITS:
                ...
                if (this.destRegionId != -1) {
                    //JOptionPane.showMessageDialog(null, "this.destination.x = " + this.destination.x + " this.destination.y = " + this.destination.y);
            ...
            case ATTACK_MOVE_UNITS_SURROUND:
                ...
                if (this.destination == null) {
                    ...
                    ArrayList<String> p = OntologyInfo.getInstance().getAdjacentRegions(destRegionId);
                    Point tmpDest;
                    for (String s : p) {
                        // s will be of the form "region 3", so we do a little parsing to get just "3" and turn that into an integer
                        tmpDest = OntologyInfo.getInstance().getRegionCenterPosition(Integer.parseInt(s.replaceAll("region", "").trim()));
...
//                result += " DEST " + this.destination;
    ...
    // returns true iff this has already been sent to the bot
	public boolean queryHasBeenSentToBot() {
		return sentToBot;
	}

	// this should be called when a plan step has been sent to the bot
	public void notifySentToBot() {
		this.sentToBot = true;
	}
}
