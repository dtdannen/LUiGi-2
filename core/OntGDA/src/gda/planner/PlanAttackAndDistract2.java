 ...
 * @date Nov 20, 2013
 */
public class PlanAttackAndDistract2 implements Plan {

	private ArrayList<PlanStep> planSteps = new ArrayList<>();
	private GoalType goalType = GoalType.ATTACK_SURROUND_DISTRACT;
	private ArrayList<Pair<String, Integer>> unitTypeCounts;
	
	private int ID = 0;

	private ArrayList<ArrayList<Plan>> subplans = new ArrayList<>();
	private int subPlanIndex = 0; // This is the one thats being used
	private int currentPlanStepIndex = -1; // not used ATM
	private Plan parentPlan;

	private boolean finished = false;
	
	public PlanAttackAndDistract2() {
		this.goalType = GoalType.ATTACK_SURROUND_DISTRACT;
		this.subplans.add(new ArrayList<Plan>()); // first set of sub plans
		// create the distract part with wraiths
		this.subplans.get(0).add(new PlanPositionAttackAirSneak());
	...
	public boolean moveToNextPlanSteps() {
		...
		if (allfinished) {
			...
			if (subPlanIndex == 0) {
				...
				ArrayList<Integer> prevSurroundUnitIds = new ArrayList<Integer>();
				ArrayList<Integer> prevSneakUnitIds = new ArrayList<Integer>();
				for (Plan prevPlan : subplans.get(0)) {
					if (prevPlan instanceof PlanPositionAttackAirSneak) {
						prevSneakUnitIds.addAll(prevPlan.getNextStepUnitIds());
					}else{
						prevSurroundUnitIds.addAll(prevPlan.getNextStepUnitIds());
					}
				}
...
//								+ prevUnitIds.toString());
				...
				Plan newDistractPlan = new PlanAttackGroundDirect(prevSurroundUnitIds);
				Plan newAttackPlan = new PlanFinishAttackAirSneak(prevSneakUnitIds);
				newDistractPlan.setParentPlan(this);
				newAttackPlan.setParentPlan(this);
				subplans.get(1).add(newDistractPlan);
				subplans.get(1).add(newAttackPlan);
				Planner.getInstance().addPlan(newAttackPlan);
				Planner.getInstance().addPlan(newDistractPlan);
	...
	/**
	 * If one of the current subplans fail, use this method to notify the hierarchical plan
	 * so it can continue with its other plans
	 */
	public void removeAbortedSubPlan(Plan failedPlan) {
		subplans.get(subPlanIndex).remove(failedPlan);
	}
