...
DefendScript::DefendScript(std::set<BWAPI::Unit*> unitsForCombat, BWAPI::Position pos) {
	this->unitsForCombat = unitsForCombat;
	this->destination = pos;
}

void DefendScript::update() {
	...
	BOOST_FOREACH (BWAPI::Unit* e, enemy->getUnits()) {
		...
		BOOST_FOREACH (BWAPI::Unit* a, this->unitsForCombat) { 
			if (e->getPosition().getApproxDistance(a->getPosition()) < 500 || BWTA::getRegion(e->getPosition())) {
...
BWAPI::Unit* DefendScript::findClosestEnemyUnitNOK(BWAPI::Unit* myUnit, std::set<BWAPI::Unit*> enemyUnits, std::map<int,int> currentAssignments) {
...
BWAPI::Unit* DefendScript::findClosestEnemyUnit(BWAPI::Unit* myUnit, std::set<BWAPI::Unit*> enemyUnits) {
	if (enemyUnits.empty()) {
		return NULL;
	}
