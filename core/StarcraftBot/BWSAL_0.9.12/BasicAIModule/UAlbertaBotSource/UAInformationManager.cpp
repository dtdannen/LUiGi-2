#include "Common.h"
#include "UAInformationManager.h"

#define SELF_INDEX 0
#define ENEMY_INDEX 1

// constructor
UAInformationManager::UAInformationManager() 
	: goForIt(false)
	, map(BWAPI::Broodwar)
...
UAInformationManager & UAInformationManager::Instance() 
{
	static UAInformationManager instance;
	return instance;
}

void UAInformationManager::update() 
{
	updateUnitInfo();
...
void UAInformationManager::updateUnitInfo() 
{
...
void UAInformationManager::initializeRegionInformation() 
{
...
void UAInformationManager::updateBaseLocationInfo() 
{
...
void UAInformationManager::updateOccupiedRegions(BWTA::Region * region, BWAPI::Player * player) 
{
...
int UAInformationManager::getIndex(BWAPI::Player * player)
{
	return player == BWAPI::Broodwar->self() ? SELF_INDEX : ENEMY_INDEX;
}

bool UAInformationManager::isEnemyBuildingInRegion(BWTA::Region * region) 
{
...
int UAInformationManager::numEnemyUnitsInRegion(BWTA::Region * region) 
{
...
const UIMap & UAInformationManager::getUnitInfo(BWAPI::Player * player) const
{
	return getUnitData(player).getUnits();
}

int UAInformationManager::numEnemyFlyingUnitsInRegion(BWTA::Region * region) 
{
	int unitsInRegion(0);
	BOOST_FOREACH(BWAPI::Unit * unit, BWAPI::Broodwar->enemy()->getUnits()) 
	{
...
std::set<BWTA::Region *> & UAInformationManager::getOccupiedRegions(BWAPI::Player * player)
{
	return occupiedRegions[getIndex(player)];
}

BWTA::BaseLocation * UAInformationManager::getMainBaseLocation(BWAPI::Player * player) 
{
	return mainBaseLocations[getIndex(player)];
}

void UAInformationManager::drawUnitInformation(int x, int y) {
...
void UAInformationManager::onStart() {}

void UAInformationManager::updateUnit(BWAPI::Unit * unit)
{
...
// is the unit valid?
bool UAInformationManager::isValidUnit(BWAPI::Unit * unit) 
{
...
void UAInformationManager::onUnitDestroy(BWAPI::Unit * unit) 
{ 
	// erase the unit
	if (unit->getPlayer() == BWAPI::Broodwar->enemy())
	{
...
BWAPI::Unit * UAInformationManager::getClosestUnitToTarget(BWAPI::UnitType type, BWAPI::Position target)
{
...
bool UAInformationManager::isCombatUnit(BWAPI::UnitType type) const
{
...
void UAInformationManager::getNearbyForce(std::vector<UnitInfo> & unitInfo, BWAPI::Position p, BWAPI::Player * player, int radius) 
{
...
bool UAInformationManager::nearbyForceHasCloaked(BWAPI::Position p, BWAPI::Player * player, int radius) 
{
...
int UAInformationManager::getNumUnits(BWAPI::UnitType t, BWAPI::Player * player)
{
	return getUnitData(player).getNumUnits(t);
}

const UnitData & UAInformationManager::getUnitData(BWAPI::Player * player) const
{
	return (player == BWAPI::Broodwar->self()) ? selfUnitData : enemyUnitData;
}

const UnitData & UAInformationManager::getUnitData(BWAPI::Unit * unit) const
{
	return getUnitData(unit->getPlayer());
}

bool UAInformationManager::enemyHasCloakedUnits()
{
	return enemyUnitData.hasCloakedUnits();
}

bool UAInformationManager::enemyHasDetector()
{
	return enemyUnitData.hasDetectorUnits();
}

bool UAInformationManager::tileContainsUnit(BWAPI::TilePosition tile)
{
