...
Squad::update()
{
	...
	if (needToRegroup)
	{
		UAInformationManager::Instance().lastFrameRegroup = 1;
...
Squad::needsToRegroup()
{
	...
	sim.setCombatUnits(unitClosest->getPosition(), Options::Micro::COMBAT_REGROUP_RADIUS + UAInformationManager::Instance().lastFrameRegroup*300);
