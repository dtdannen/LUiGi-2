...
UAProductionManager::UAProductionManager() 
...
void UAProductionManager::setBuildOrder(const std::vector<MetaType> & buildOrder)
{
...
void UAProductionManager::performBuildOrderSearch(const std::vector< std::pair<MetaType, UnitCountType> > & goal)
{	
...
void UAProductionManager::update() 
{
	...
	if (!enemyCloakedDetected && UAInformationManager::Instance().enemyHasCloakedUnits())
	{
...
// on unit destroy
void UAProductionManager::onUnitDestroy(BWAPI::Unit * unit)
{
	...
	if ((unit->getType().isWorker() && !UAWorkerManager::Instance().isWorkerScout(unit)) || unit->getType().isBuilding())
	{
...
void UAProductionManager::manageBuildOrderQueue() 
{
	...
	while (!queue.isEmpty()) 
	{
		...
		if (currentItem.metaType.isBuilding() && !(producer && canMake))
		{
			...
			producer = UAWorkerManager::Instance().getBuilder(b, false);
...
bool UAProductionManager::canMakeNow(BWAPI::Unit * producer, MetaType t)
{
...
bool UAProductionManager::detectBuildOrderDeadlock()
{
...
void UAProductionManager::predictWorkerMovement(const Building & b)
{
	...
	BWAPI::Unit * moveWorker			= UAWorkerManager::Instance().getMoveWorker(walkToPosition);
	...
	if (moveWorker && haveLocationForThisBuilding && !assignedWorkerForThisBuilding && (predictedTilePosition != BWAPI::TilePositions::None) &&
		UAWorkerManager::Instance().willHaveResources(mineralsRequired, gasRequired, moveWorker->getDistance(walkToPosition)) )
	{
		// we have assigned a worker
		assignedWorkerForThisBuilding = true;

		// tell the worker manager to move this worker
		UAWorkerManager::Instance().setMoveWorker(mineralsRequired, gasRequired, walkToPosition);
	}
}

void UAProductionManager::performCommand(BWAPI::UnitCommandType t) {
...
void UAProductionManager::createMetaType(BWAPI::Unit * producer, MetaType t) 
{
	if (!producer)
	{
		return;
...
BWAPI::Unit * UAProductionManager::selectUnitOfType(BWAPI::UnitType type, bool leastTrainingTimeRemaining, BWAPI::Position closestTo) {
...
void UAProductionManager::onSendText(std::string text)
{
...
void UAProductionManager::populateTypeCharMap()
{
...
void UAProductionManager::drawProductionInformation(int x, int y)
{
...
UAProductionManager & UAProductionManager::Instance() {

	static UAProductionManager instance;
	return instance;
}