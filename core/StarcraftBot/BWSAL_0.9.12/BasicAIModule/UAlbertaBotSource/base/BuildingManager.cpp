...
void BuildingManager::update() 
{
	...
	//UABuildingPlacer::Instance().drawReservedTiles();
...
void BuildingManager::assignWorkersToUnassignedBuildings() 
{
	...
	while (buildingData.hasNextBuilding(ConstructionData::Unassigned)) 
	{
		...
		// grab a worker unit from UAWorkerManager which is closest to this final position
		BWAPI::Unit * workerToAssign = UAWorkerManager::Instance().getBuilder(b);
		...
		if (workerToAssign) {
			...
			// reserve this space
			UABuildingPlacer::Instance().reserveTiles(b.finalPosition, b.type.tileWidth(), b.type.tileHeight());
...
BWAPI::TilePosition BuildingManager::getBuildingLocation(const Building & b)
{
	...
	{
		return UABuildingPlacer::Instance().getRefineryPosition();
	}

	// special case for early pylons
	if (b.type == BWAPI::UnitTypes::Protoss_Pylon && (numPylons < 3))
	{
		BWAPI::TilePosition posInRegion =    UABuildingPlacer::Instance().getBuildLocationNear(b, 4, true);
		BWAPI::TilePosition posNotInRegion = UABuildingPlacer::Instance().getBuildLocationNear(b, 4, false);
	...
	else
	{
		...
		else
		{
			...
			BWAPI::TilePosition posInRegion =    UABuildingPlacer::Instance().getBuildLocationNear(b, distance, true,  horizontalOnly);

			// get a region anywhere
			BWAPI::TilePosition posNotInRegion = UABuildingPlacer::Instance().getBuildLocationNear(b, distance, false, horizontalOnly);
	...
	while(buildingData.hasNextBuilding(ConstructionData::Assigned)) 
	{
		...
		if (!b.builderUnit->isConstructing()) 
		{
			...
			else if (b.buildCommandGiven) 
			{
				...
				UAWorkerManager::Instance().finishedWithWorker(b.builderUnit);

				// free the previous location in reserved
				UABuildingPlacer::Instance().freeTiles(b.finalPosition, b.type.tileWidth(), b.type.tileHeight());
