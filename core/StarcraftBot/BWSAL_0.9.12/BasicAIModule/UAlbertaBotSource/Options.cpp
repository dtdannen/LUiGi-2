...
namespace Options
{
	...
	{
		const bool USING_GAMECOMMANDER			= false;	// toggle GameCommander, effectively UAlbertaBot
		...
		const bool USING_MICRO_SEARCH			= true;	// toggle use of Micro Search, if false script used
		const bool USING_MACRO_SEARCH			= false;		// toggle use of Build Order Search, currently no backup
	...
	namespace Micro								// micromanagement options
	{
		...
		int MICRO_SEARCH_MAX_TIMER				= 200;		// maximum amount of time to allot to micro search (ms)
		int MICRO_SEARCH_MIN_TIMER				= 100;		// minimum amount of time to allot to micro search (ms)
	}
