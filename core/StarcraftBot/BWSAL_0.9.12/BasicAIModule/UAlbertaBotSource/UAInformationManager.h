...
class UAInformationManager {

	UAInformationManager();
...
struct BaseInfo {
	...
	bool isBuiltOn() {
		...
		const UnitInfoVector & ourUnits = UAInformationManager::Instance().getUnitInfo(BWAPI::Broodwar->self());
		const UnitInfoVector & enemyUnits = UAInformationManager::Instance().getUnitInfo(BWAPI::Broodwar->self());
